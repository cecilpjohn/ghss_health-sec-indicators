                    ...
                    dashboardBody(
                      ...
                      fluidRow(column(width = 12,
                                      ...
                                      column(width = 10, 
                                             br(),offset = 1,
                                             h2("Overview"),
                                             p("The dashboard is a compilation of global health security and 
                             financing indicators. The respective data sources for each indicator
                             can be accessed directly by clicking on title of the chart. The charts
                             reflect data as retrieved in May 2023 from the respective data sources."),
                                             br()
...
server <- function(input, output, session) {
  ...
  output$plot_cc_funding <- renderPlot({ 
    ...
    ###Set string length for labels
    dfplot_cc_funding$`core capacity` <- str_wrap(dfplot_cc_funding$`core capacity`,
                                                  width = 20)
    
    ###Plot for core capacities
    ggplot(dfplot_cc_funding,aes(x = `total disbursed`, 
                                 y = reorder(`core capacity`, `total disbursed`)))+
      geom_bar(stat = "identity", fill = "#001F3F", width = 0.7) +
      geom_text(aes(label = label_number_si()(`total disbursed`)),hjust = -0.3) +
      ...
      theme(
        ...
        panel.grid.major.y = element_line(color = "#ced4da", size = 0.4),
        panel.grid.major.x = element_blank(),
        axis.ticks.length = unit(0, "mm"),
        axis.title = element_blank(),
        axis.line.y.left  = element_line(color = "#000000", size = 1.5),
  ...
  output$ccfundingBox <- renderValueBox({
    ...
    format_number <- function(x) {
      if (abs(x) >= 10^9) {
        paste(format(round(x / 10^9, 2), nsmall =2), "B")
      } else if (abs(x) >= 10^6) {
        paste(format(round(x / 10^6, 2), nsmall =2), "M")
      } else if (abs(x) >= 10^3) {
        paste(format(round(x / 10^3, 2), nsmall =2), "K")
      } else {
        as.character(x)
      }
    }
    ...
    valueBox(
      value = format_number(sum(dfplot_total_funding$`total funding disbursed`,
                                na.rm=T)), 
  ...
  output$pheicfundingBox <- renderValueBox({
    ...
    valueBox(
      value = format_number(sum(dfplot_pheic$`Total disbursed to recipient`,
                                na.rm=T)),
  ...
  output$plot_pheic <- renderPlot({
    ...
    ###Set string lenght for labels
    dfplot_pheic$`PHEIC name` <- str_wrap(dfplot_pheic$`PHEIC name`, width = 10)
    
    ###Create plot for country
    ggplot(dfplot_pheic) +
      geom_col(aes((`PHEIC name`), `Total disbursed to recipient`), 
               fill = "#001F3F", width = 0.4) +
      geom_text(aes(x=`PHEIC name`,  y=`Total disbursed to recipient`, 
                    label = (label_number_si()(`Total disbursed to recipient`)))
                ,vjust = -0.7)+
      scale_x_discrete()+
      scale_y_continuous(
      ...
      theme(
        panel.background = element_rect(fill = "white"),
        panel.grid.major.x = element_line(color = "#ced4da", size = 0.4),
        axis.ticks.length = unit(0, "mm"),
        axis.title = element_blank(),
  ...
  output$plot_sdg <- renderPlot({
    ...
    ggplot(data = df, aes(x = x, y = 1)) +
      ...
      theme(
        text = element_text(family = "IBM Plex Sans", color = "#000000"),
        aspect.ratio=1/1,
  ...
  output$plot_gini <- renderPlot({
    ...
    ggplot(gini_country, aes(x = x, y = value, fill = name)) +
      geom_col(show.legend = FALSE) +
      geom_text(x=-2, aes(label=paste(dfplot_gini$Gini)))+
  ...
  output$plot_corruption <- renderPlot({
    ...
    corruption_country <- tibble(x = 1, y = score_corruption/100) %>% 
      mutate(y_negative = 1 - y) %>% 
      pivot_longer(cols = -x) 
    
    ggplot(corruption_country, aes(x = x, y = value, fill = name)) +
      geom_col(show.legend = FALSE) +
      geom_text(x=-2, aes(label=paste(dfplot_corruption$`CPI score 2022`)))+
      coord_polar(theta = "y",direction = -1) +
      xlim(c(-2, 2)) +
      scale_fill_manual(values = c("#001F3F", "#ced4da")) +
      theme_void()+
      labs(title = paste("Rank",dfplot_corruption$Rank,"/", 
                         max(df_corruption$Rank, na.rm = TRUE)))+
      theme(
        text = element_text(family = "IBM Plex Sans", color = "#000000"),
        aspect.ratio=1/1,
        plot.title = element_text(hjust=0.5))
  ...
  output$plot_spar <- renderPlot({
    ...
    dfplot_spar_long$Capacity <- str_wrap(dfplot_spar_long$Capacity, width = 12)
    
    ###plot for spar
    ggplot(dfplot_spar_long, aes(x=Capacity, y=Score, fill=category))+
      ...
      theme(
        text = element_text(color = "#000000", family = "IBM Plex Sans"),
        aspect.ratio=2/2,
        ...
        panel.grid.major.x = element_line(color = "#ced4da",size = 0.5, linetype = 2),
        legend.position = "None")+
  ...
  output$plot_jee <- renderPlot({
    ...
    dfplot_jee$capacity <- str_wrap(dfplot_jee$capacity, width = 20)
    
    ggplot(dfplot_jee, aes(x=capacity, y=score, color = category))+
      geom_segment(aes(x=capacity, xend=capacity, y=0, yend=score), 
                   color="#ced4da", 
                   linewidth=0.5)+
      geom_point(size=4)+
      ...
      theme(
        ...
        panel.grid.major.x = element_line(color = "#FFB30F", size = 0.5, linetype = 2),
        panel.grid.minor = element_blank(),
        panel.border = element_blank(),
        axis.ticks.x = element_blank(),
        axis.text.x = element_blank(),
        axis.text.y = element_text(family = "IBM Plex Sans"),
        axis.line.x.bottom = element_line(color = "#000000", size = 2),
      ...
      xlab("")+
      ylab("")+
      labs(subtitle = paste("Scores from",max(dfplot_jee$year)))
  ...
  output$plot_ghsindex <- renderPlot({
    ...
    ###plot ghsindex
    ggplot(df_ghsindex, aes( x=`OVERALL SCORE`))+
      geom_histogram(fill = "#ced4da" , alpha=1, bins=50, color = "white")+
      geom_vline(xintercept = dfplot_ghsindex$`OVERALL SCORE`, 
                 color = "#001F3F", linewidth = 1.5, linetype ="dashed")+
      ...
      labs(
        title = paste("Rank",dfplot_ghsindex$RANK,"out of",
  ...
  output$plot_rand <- renderPlot({
    ...
    ggplot(df_rand, aes( x=`Overall Score Normed`))+
      ...
      labs(
        title = paste("Rank",dfplot_rand$Rank,"out of",
  ...
  output$plot_inform <- renderPlot({
    ...
    ggplot(df_inform, aes(x=`INFORM RISK`))+
      geom_histogram(fill="#ced4da", 
                     alpha=1, 
                     bins=50, 
                     color = "white")+
      ...
      labs(
        title = paste("Score:",dfplot_inform$`INFORM RISK`))
  ...
  output$naphs_table <- DT::renderDataTable({
    ...
    dfplot_naphs$Documents <- ifelse(is.na(dfplot_naphs$Status), "No Records Found", 
                                     ifelse(is.na(dfplot_naphs$Documents), "NA",
                                            paste0("<a href='",dfplot_naphs$Documents,
                                                   "'>",dfplot_naphs$Documents,"</a>")))
    
    datatable(dfplot_naphs, options = list(dom = 't'), escape = FALSE)
  ...
  output$pvs_table <- DT::renderDataTable({
    ...
    dfplot_pvs <- df_pvs %>% 
      filter(Economy == input$country) %>% 
      select(`Report Type`, Version, Year, `Report Link`)
    
    dfplot_pvs$`Report Link` <- ifelse(is.na(dfplot_pvs$`Report Type`), "No Records Found",
                                       ifelse(is.na(dfplot_pvs$`Report Link`),"NA",
                                              paste0("<a href='",dfplot_pvs$`Report Link`,
                                                     "'>",dfplot_pvs$`Report Link`,"</a>")))
    
    datatable(dfplot_pvs, options = list(dom = 't'), escape = FALSE)
  })
